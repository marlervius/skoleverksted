...
def _resolve_source_details(req, ctx: "JobContext") -> tuple[Optional[str], Optional[str], Optional[str]]:
    ...
    if getattr(req, "use_ndla", False):
        ...
        if ndla:
            ...
            return ndla["text"], f"NDLA: {ndla['title']}", ndla.get("url")
...
def _resolve_source(req, ctx: "JobContext") -> tuple[Optional[str], Optional[str]]:
    """Backward-compatible source helper for callers that only need labels."""
    text, name, _url = _resolve_source_details(req, ctx)
    return text, name
...
def _lesson_worker(ctx: JobContext) -> tuple[bytes, str]:
    ...
    if req.basis_text:
        source_text, source_name, source_url = req.source_text, ("lærerens kildemateriale" if req.source_text else None), None
    else:
        source_text, source_name, source_url = _resolve_source_details(req, ctx)
    provided_sources = (
        [{"title": source_name or "Kilde", "url": source_url, "origin": "grounding", "fetch_status": "grounded"}]
        if source_url else []
    )
