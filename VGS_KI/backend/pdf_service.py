...
def compile_typst(source: str, image_path: Optional[str] = None) -> bytes:
    ...
    """
    # Allow production images to pin the executable explicitly while keeping
    # local PATH discovery as the default.
...
'''
    ...
    image_section = ""
    if image_path:
        safe_image_caption = sanitize_for_typst(image_caption)
        safe_image_credit = sanitize_for_typst(
            image_credit or ("Source: Wikimedia Commons" if is_english else "Kilde: Wikimedia Commons")
        )
        caption_line = (
            f'#text(size: 9pt, fill: gray, style: "italic")[{safe_image_caption}]\n'
            "    #linebreak()\n"
            if safe_image_caption
            else ""
        )
        image_section = f'''
// Featured pedagogical image - kept together with caption and source
#block(breakable: false)[
  #align(center)[
    ...
    #v(0.3em)
    {caption_line}#text(size: 7.5pt, fill: gray)[{safe_image_credit}]
  ]
]
...
def create_differentiated_pdf(
    ...
    if image_filename:
        safe_image_caption = sanitize_for_typst(image_caption)
        safe_image_credit = sanitize_for_typst(image_credit or "Kilde: Wikimedia Commons")
        caption_line = (
            f'#text(size: 9pt, fill: gray, style: "italic")[{safe_image_caption}]\n'
            "  #linebreak()\n"
            if safe_image_caption
