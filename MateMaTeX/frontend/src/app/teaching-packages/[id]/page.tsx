...
const statusLabels: Record<string, string> = {
  ...
  user_approved_with_exceptions: "Eldre godkjenning – må kontrolleres på nytt",
...
export default function TeachingPackagePage({ params }: { params: { id: string } }) {
  ...
  const finalApproved = pkg.status === "approved";
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-6">
      ...
      <section className="card flex flex-wrap items-center justify-between gap-4 border-accent-blue/30"><div><h2 className="text-lg font-semibold">Godkjenn hele pakken</h2><p className="mt-1 text-sm text-text-secondary">Alle obligatoriske artefakter må være kildekontrollert og lærer-godkjent på samme revisjon. Karantenepunkter er utelatt fra alle eksportfiler og må bekreftes uttrykkelig.</p>{packageReasons.length > 0 && <p className="mt-2 text-xs text-accent-orange">Åpne krav: {packageReasons.slice(0, 5).join(" · ")}</p>}{(unresolvedCount + quarantinedCount) > 0 && <p className="mt-2 text-xs text-accent-orange">{unresolvedCount + quarantinedCount} punkt(er) krever lærervurdering eller er utelatt fra eksport.</p>}{pkg.approval_history?.length > 0 && <p className="mt-2 text-xs text-text-muted">Siste beslutning: {statusLabels[pkg.approval_history[pkg.approval_history.length - 1].action] || pkg.approval_history[pkg.approval_history.length - 1].action} · {pkg.approval_history[pkg.approval_history.length - 1].reason}</p>}{(unresolvedCount + quarantinedCount) > 0 && !finalApproved && <div className="mt-3 w-full rounded-lg border border-accent-orange/30 bg-accent-orange/5 p-3"><p className="text-xs font-semibold text-accent-orange">Les karantenelisten og beskriv konsekvensen av at punktene ikke blir med i elevmaterialet.</p><textarea className="mt-2 min-h-20 w-full rounded-lg border border-border bg-surface p-2 text-sm" value={exceptionReason} onChange={(event) => setExceptionReason(event.target.value)} placeholder="Bekreft hvorfor utelatelsene er faglig forsvarlige …" aria-label="Begrunnelse for utelatelser" /></div>}</div><div className="flex flex-wrap gap-2"><button className="btn-primary" onClick={() => void action("approve-package", () => approveTeachingPackage(pkg.id, teacher))} disabled={busy !== "" || packageReasons.length > 0 || unresolvedCount + quarantinedCount > 0 || finalApproved}><CheckCircle2 className="h-4 w-4" /> Godkjenn og legg i årsplanen</button>{(unresolvedCount + quarantinedCount) > 0 && !finalApproved && <button className="btn-secondary" onClick={() => void action("approve-exceptions", () => approveTeachingPackageWithOmissions(pkg.id, teacher, exceptionReason.trim()))} disabled={busy !== "" || !exceptionReason.trim()}><ShieldCheck className="h-4 w-4" /> Bekreft utelatelser og godkjenn</button>}{finalApproved && <a className="btn-secondary" href={teachingPackageZipDownloadUrl(pkg.id)}><Download className="h-4 w-4" /> Last ned ZIP</a>}</div></section>
    </main>
  );
}
