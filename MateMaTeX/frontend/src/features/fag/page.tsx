...
export default function Home() {
  ...
  const DRAFT_KEY = "skoleverksted_fag_draft_v1";
  ...
  useEffect(() => {
    try {
      ...
      if (typeof draft.topic === "string") dispatch({ type: "SET_TOPIC", topic: draft.topic });
  ...
  // Prefill when opened from a cross-module project or theme pack.
  useEffect(() => {
                ...
                <div className="flex flex-col gap-3 rounded-xl border border-amber-300 bg-amber-50 p-4">
                  ...
                  {qualityQuarantine.length > 0 && (
                    <div className="rounded-lg border border-amber-200 bg-white p-3 text-sm text-amber-950">
                      <p className="font-semibold">Karantene ({qualityQuarantine.length})</p>
                      <ul className="mt-1 list-disc space-y-1 pl-5">
                        {qualityQuarantine.slice(0, 8).map((item, index) => (
